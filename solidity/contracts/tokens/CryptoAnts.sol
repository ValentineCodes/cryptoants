    ...
    ConfirmedOwner
{
    ...
    mapping(uint256 antId => uint256 ovipositionPeriod)
        private s_ovipositionPeriod;

    // oviposition requests
    mapping(uint256 requestId => OvipositionRequest ovipositionRequest)
        private s_ovipositionRequests;
   ...
   */
    function startOviposition(
        ...
        if (
            block.timestamp >= ovipositionPeriod &&
            block.timestamp < ovipositionPeriod + OVIPOSITION_DEADLINE
        ) {
            // cost of request
            uint256 paid = VRF_V2_WRAPPER.calculateRequestPrice(
                CALLBACK_GAS_LIMIT
            );
            ...
            s_ovipositionRequests[requestId] = OvipositionRequest({
                paid: paid,
                fulfilled: false,
                ant: Ant({owner: msg.sender, id: _antId})
            });
    ...
    function fulfillRandomWords(
        ...
        OvipositionRequest storage ovipositionRequest = s_ovipositionRequests[
            _requestId
        ];
    ...
    function _layEggs(
        Ant memory ant,
        uint256 _eggsFertilized,
        uint256 _dyingChance,
   ...
   */
    function updatePrices(
        uint256 _newEggPrice,
        uint256 _newAntPrice
    ) external onlyOwner {
