...
contract E2ECryptoAnts is Test, TestUtils {
    ...
    function setUp() public {
        ...
        governanceTimeLock = new GovernanceTimeLock(
            MIN_DELAY,
            proposers,
            executors
        );
        ...
        if (linkToken.transfer(address(ants), AMOUNT_TO_FUND_ANTS) == false)
    ...
    function testCanUpdatePrices() public {
        ...
        executeGovernanceFunction(
            CryptoAnts.updatePrices.selector,
            abi.encode(NEW_EGG_PRICE, NEW_ANT_PRICE)
        );
    ...
    function testWithdrawEther() public {
        ...
        executeGovernanceFunction(
            CryptoAnts.withdrawEther.selector,
            abi.encode(alice, ants.getEggPrice())
        );
    ...
    function testWithdrawLink() public {
        ...
        executeGovernanceFunction(
            CryptoAnts.withdrawLink.selector,
            abi.encode(alice, prevAntsLinkBalance)
        );

        assertEq(
            linkToken.balanceOf(alice),
            prevAliceBalance + prevAntsLinkBalance
        );
    ...
    function executeGovernanceFunction(
